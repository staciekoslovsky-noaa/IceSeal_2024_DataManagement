# Ice Seals 2024: Identify empty frames
# S. Koslovsky
...
images <- RPostgreSQL::dbGetQuery(con, "SELECT image_dir || \'\\' || image_name AS image FROM surv_ice_seals_2024.tbl_images WHERE image_type = \'rgb_image\'")
...
RPostgreSQL::dbSendQuery(con, "ALTER TABLE surv_ice_seals_2024.tbl_images ADD COLUMN IF NOT EXISTS rgb_imagezero VARCHAR(10)")
RPostgreSQL::dbSendQuery(con, "UPDATE surv_ice_seals_2024.tbl_images SET rgb_imagezero = NULL")

for (i in 1:nrow(file_details2DB)) {
  sql <- paste0("UPDATE surv_ice_seals_2024.tbl_images SET rgb_imagezero = \'", file_details2DB$rgb_imagezero[i], 
...
RPostgreSQL::dbSendQuery(con, paste0("UPDATE surv_ice_seals_2024.tbl_images SET rgb_imagezero = \'NA\' WHERE rgb_imagezero IS NULL"))
