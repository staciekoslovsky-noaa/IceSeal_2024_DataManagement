# Ice Seals 2024: Process Data/Images to DB
# S. Koslovsky

# Set Working Variables
wd <- "//akc0ss-n086/NMML_Polar_Imagery_2/Surveys_IceSeals_2024"
metaTemplate <- "//akc0ss-n086/NMML_Polar_Imagery_2/Surveys_IceSeals_2024/Template4Import.json"
projectPrefix <- "ice_seals_2024"
...
for (i in 1:length(dat)){
  sql <- paste("SELECT fxn_deps_save_and_drop_dependencies(\'surv_ice_seals_2024', \'", dat[i], "\')", sep = "")
...
for (i in 1:length(dat)){
  RPostgreSQL::dbWriteTable(con, c("surv_ice_seals_2024", dat[i]), data.frame(df[i]), overwrite = TRUE, row.names = FALSE)
  if (i == 2) {
    sql1 <- paste("ALTER TABLE surv_ice_seals_2024.", dat[i], " ADD COLUMN geom geometry(POINT, 4326)", sep = "")
    sql2 <- paste("UPDATE surv_ice_seals_2024.", dat[i], " SET geom = ST_SetSRID(ST_MakePoint(ins_longitude, ins_latitude), 4326)", sep = "")
...
for (i in length(dat):1) {
  sql <- paste("SELECT fxn_deps_restore_dependencies(\'surv_ice_seals_2024\', \'", dat[i], "\')", sep = "")
  RPostgreSQL::dbSendQuery(con, sql)
  RPostgreSQL::dbClearResult(dbListResults(con)[[1]])
}
