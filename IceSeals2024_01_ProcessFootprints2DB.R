...
RPostgreSQL::dbSendQuery(con, "DELETE FROM surv_ice_seals_2024.geo_images_footprint")
...
for (j in 1:nrow(dir)) {
  ...
  for (i in 1:length(shps)) {
    next_id <- RPostgreSQL::dbGetQuery(con, "SELECT max(id) FROM surv_ice_seals_2024.geo_images_footprint")
    ...
    # Write data to DB
    sf::st_write(shape, con, c("surv_ice_seals_2024", "geo_images_footprint"), append = TRUE)
  }
}
